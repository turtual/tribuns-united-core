...
function HeroIllustration() {
  return (
    ...
    >
      ...
      <path d="M150 78 C170 60 190 60 205 78" strokeDasharray="2 6" opacity="0.55" />
...
function Spinner() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="animate-spin" aria-hidden="true">
      ...
      <path d="M21 12 A9 9 0 0 0 12 3" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    </svg>
  );
}
    ...
    <div className="min-h-screen bg-bg-primary text-text-primary relative">
      <div className="mx-auto w-full max-w-[393px] min-h-screen flex flex-col px-6 pt-14 pb-8">
        <header className="relative h-10 flex items-center justify-center">
          <button type="button" aria-label="Geri" className="absolute left-0 size-10 -ml-2 flex items-center justify-center text-text-primary">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M15 5 L8 12 L15 19" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <h1 className="font-display text-text-primary" style={{ fontWeight: 600, fontSize: 20, lineHeight: "28px" }}>
            Kimlik Doğrulama
          </h1>
        </header>
        ...
        <section className="mt-2">
          <h2 className="font-display text-text-primary" style={{ fontWeight: 600, fontSize: 24, lineHeight: "32px", letterSpacing: "-0.01em" }}>
            Tribüne girmek için kimliğini doğrula
          </h2>
          <p className="mt-3 text-text-secondary" style={{ fontFamily: "var(--font-sans)", fontWeight: 400, fontSize: 14, lineHeight: "20px" }}>
            TRİBÜN gerçek taraftarların alanı. Botları ve sahte hesapları engellemek için TC kimlik numarası ve selfie istiyoruz. Bilgilerin şifrelenir, hiçbir yerde paylaşılmaz.
          </p>
        </section>

        <section className="mt-5 bg-surface rounded-xl border flex flex-col gap-4" style={{ borderColor: "var(--border-tertiary)", padding: 20 }}>
          <label className="flex flex-col gap-1.5">
            <span className="text-text-secondary" style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 12, lineHeight: "16px" }}>
              TC Kimlik No
            </span>
            <input
              inputMode="numeric"
              ...
              style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 18, letterSpacing: "0.18em", paddingBottom: 6, borderColor: "var(--border)" }}
            />
          </label>
          ...
          {!selfieReady ? (
            <button
              ...
              style={{ borderColor: "var(--border)", height: 52, fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 15 }}
            >
              <CameraGlyph />
              <span>Selfie çek</span>
            </button>
          ) : (
            <div className="w-full rounded-pill flex items-center gap-3 px-2" style={{ backgroundColor: "var(--success-bg)", height: 52 }}>
              <div className="size-10 rounded-pill overflow-hidden flex items-center justify-center" style={{ backgroundColor: "var(--bg-tertiary)" }} aria-hidden="true">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                  <circle cx="12" cy="9.5" r="3.5" stroke="var(--text-secondary)" strokeWidth="1.6" />
                  <path d="M5 20 C7 16 10 14.5 12 14.5 C14 14.5 17 16 19 20" stroke="var(--text-secondary)" strokeWidth="1.6" strokeLinecap="round" />
                </svg>
              </div>
              <span className="flex-1" style={{ fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 14, color: "var(--success)" }}>
                Selfie hazır
              </span>
              <span className="size-7 rounded-pill flex items-center justify-center mr-1" style={{ backgroundColor: "var(--success)", color: "white" }}>
                <CheckGlyph size={16} />
              </span>
            </div>
          )}
        ...
        <div className="mt-auto pt-6">
          <button
            ...
            style={{ height: 56, fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 16, letterSpacing: "0.01em" }}
          >
            {status === "verifying" ? (
              <>
                <Spinner />
...
function SuccessOverlay({ onContinue }: { onContinue: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6" style={{ backgroundColor: "color-mix(in oklab, var(--bg-primary) 92%, black 8%)" }}>
      <div className="w-full max-w-[345px] bg-surface rounded-2xl flex flex-col items-center text-center px-6 pt-8 pb-6 border" style={{ borderColor: "var(--border-tertiary)" }}>
        <div className="size-20 rounded-pill flex items-center justify-center" style={{ backgroundColor: "var(--success-bg)", color: "var(--success)" }}>
          <CheckGlyph size={44} />
        </div>
        <h2 className="mt-5 font-display text-text-primary" style={{ fontWeight: 600, fontSize: 24, lineHeight: "32px", letterSpacing: "-0.01em" }}>
          Hoş geldin Tribüne
        </h2>
        <p className="mt-2 text-text-secondary" style={{ fontFamily: "var(--font-sans)", fontSize: 14, lineHeight: "20px" }}>
        ...
        <button
          ...
          style={{ height: 52, fontFamily: "var(--font-sans)", fontWeight: 500, fontSize: 16 }}
        >
          Devam et
        </button>
      </div>
